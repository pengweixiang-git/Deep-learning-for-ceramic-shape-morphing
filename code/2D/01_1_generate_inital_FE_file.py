...
while jobnunmber <= 232:
    ...
    mdb.models['Model-1'].materials['xy-expansion'].Expansion(dependencies=0,
                                                              table=((-0.08, -0.08, -0.16, 0.0, 0.0, 0.0),),
    ...
    myModel.Material(name='yz-90-expansion')
    myModel.materials['yz-90-expansion'].setValues(description='')
    myModel.materials['yz-90-expansion'].Elastic(dependencies=0, moduli=LONG_TERM, noCompression=OFF, noTension=OFF,
                                                 table=((18600.0, 0.22),),
                                                 temperatureDependency=OFF, type=ISOTROPIC)
    myModel.materials['yz-90-expansion'].Density(dependencies=0, distributionType=UNIFORM, fieldName='',
                                                 table=((3.95e-09,),),
                                                 temperatureDependency=OFF)
    myModel.materials['yz-90-expansion'].Expansion(dependencies=0, table=((-0.08, -0.16, -0.08, 0.0, 0.0, 0.0),),
                                                   temperatureDependency=OFF, type=
                                                   ANISOTROPIC, userSubroutine=OFF, zero=0.0)
    myModel.materials['yz-90-expansion'].setValues(materialIdentifier='')


    myModel.HomogeneousSolidSection(material='xy-expansion', name='Section-1', thickness=None)
    myModel.HomogeneousSolidSection(material='yz-expansion', name='Section-2', thickness=None)
    myModel.HomogeneousSolidSection(material='yz-90-expansion', name='Section-3', thickness=None)
    ...
    myModel.parts['Part-all'].seedPart(size=1.25, deviationFactor=0.1, minSizeFactor=0.1)
    myModel.parts['Part-all'].generateMesh()

    myModel.parts['Part-all'].Set(name='Set-point-1', nodes=
    myModel.parts['Part-all'].nodes.getSequenceFromMask(mask=(
        '[#1 ]',), ))
    myModel.parts['Part-all'].Set(name='Set-point-x', nodes=
    myModel.parts['Part-all'].nodes.getSequenceFromMask(mask=(
        '[#200000 ]',), ))
    myModel.parts['Part-all'].Set(name='Set-point-y', nodes=
    myModel.parts['Part-all'].nodes.getSequenceFromMask(mask=(
        '[#0 #10000000 ]',), ))

    # myModel.parts['Part-all'].Set(name='Set-node', nodes=
    # myModel.parts['Part-all'].nodes.getSequenceFromMask(mask=(
    # '[#9 #0:6 #40000 #0:3 #10810800 #0:40 #8080000',
    # ' #0:20 #10080 #0:55 #8000 ]', ), ))
    #
    # myModel.EncastreBC(createStepName='Initial', localCsys=None,
    #                    name='BC-1', region=myAssembly.instances['Part-all-1'].sets['Set-node'])

    myModel.EncastreBC(createStepName='Initial', localCsys=None, name='BC-1',
                       region=myAssembly.instances['Part-all-1'].sets['Set-point-1'])
    myModel.ZsymmBC(createStepName='Initial', localCsys=None, name='BC-2',
                    region=myAssembly.instances['Part-all-1'].sets['Set-point-x'])
    myModel.YsymmBC(createStepName='Initial', localCsys=None, name='BC-3',
                    region=myAssembly.instances['Part-all-1'].sets['Set-point-x'])
    myModel.XsymmBC(createStepName='Initial', localCsys=None, name='BC-4',
                    region=myAssembly.instances['Part-all-1'].sets['Set-point-y'])
    myModel.ZsymmBC(createStepName='Initial', localCsys=None, name='BC-5',
                    region=myAssembly.instances['Part-all-1'].sets['Set-point-y'])
